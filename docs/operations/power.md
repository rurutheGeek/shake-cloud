...
**ホスト自体を意図して `reboot`／`shutdown` したときは別扱いにしています。**
`pve-guests` の停止処理（`stopall`）は、その時点で動いている全ゲストへ
`qmshutdown` を送ります。これはタスクログ上、「オペレーターがそのゲストだけ
...
`save-shutdown` を呼びます。これは通常の保存に加えて印（
`/var/lib/shakecloud/clean-shutdown`）を残し、次の `restore` はその印がある
間だけタスクログの判定を飛ばして保存された組をそのまま起こします。印は
`restore` が読んだ時点で消費されるので、次に本当の停電・ハングが起きたときは
従来どおりタスクログ判定に戻ります。毎分の定期 `save` も、途中で止まった
`restore` が残した古い印を毎回消すので、印が誤って生き残ることはありません。
