...
- media-01は新規cloud VMにNextcloud・Calendar・Tasks・Kavita・Navidrome・MeTube・Picard・LocalSend・Nextcloud印刷を載せます。機能群の停止・再開をVM単位で行います。**2026-09-12に配備済みで、既存環境からのデータ移行が未完です。**RomMはゲームVM（game1）へ載せ、メディアの機能群とは分けます。
