...
- media-01は新規cloud VMにNextcloud・Calendar・Tasks・Kavita・Navidrome・MeTube・Picard・LocalSend・Nextcloud印刷を載せます。機能群の停止・再開をVM単位で行います。**2026-09-12に配備済みで、旧環境からのデータ移行が未完です。**RomMはゲームVM（game1）へ載せ、メディアの機能群とは分けます。
...
- 旧環境（ハブ）からのメディアデータ移行（[並列開発計画](../development/index.md)のW03〜W06）。
