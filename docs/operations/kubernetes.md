...
- 関数 URL は `<name>.functions.k8s.apextox.dpdns.org`。ワイルドカード `*.functions.k8s`（`dns.yaml`、Kourier の LB）を向けてあり、そのまま HTTP で叩けます（TLS は未対応）。
...
1. 関数 URL の TLS（Knative + cert-manager）
2. CNPG のバックアップを Garage（S3）へ
3. Phase 8 の VLAN 実機切替（物理スイッチ/ルータ）
