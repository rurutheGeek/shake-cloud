...
| ドキュメントサイト | services-01 に置いて LAN に公開（`https://docs.apextox.dpdns.org`）。Git の `docs/` が正本で、旧ハブの「Nextcloud で編集する」仕組みは持ち込まない。今回の方針ではservices-01に維持する。家電（H01）は2026-09-12に配備済みで、Authentik SSO・Eufy・SwitchBot Cloud を設定済み。VPNの同居は未配備 |
...
| メディア系の旧環境（データ移行元） | 旧構成のハブ。SSH トンネル経由 | 旧 Authentik | この基盤の外。[ハブ運用](hub.md)。新しい入口は[接続先一覧](urls.md) |
...
| メディア系の認証 | **新 media-01 は新しい identity の OIDC / Forward Auth に統合済み（2026-09-12）**。旧 `stacks/hub` の Authentik はデータ移行元として残る | 旧環境の扱いはデータ移行の完了後に決める |
