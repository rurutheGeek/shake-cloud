...
| 🟨 | Windows | **ポータルから Windows 11 Pro の VM を作れるようにした（2026-09-12）。** イメージの `os: windows` を正として、API が UEFI（OVMF）・TPM 2.0・q35 の VM を作り、初回設定を cloudbase-init の NoCloud（network config v1）へ渡す。ポータルは Windows イメージを選ぶと SSH鍵の欄を隠し、コンソールでの初期設定を案内する。API・ポータル・テストは実装済みで、**cloud-01 へ配備済み**。Proxmox 実機の `windows_devices` プローブ（UEFI/TPM/q35 のVM作成と削除）と既存プローブ全6件は PASS。**ゴールデンイメージの作成（Win11 Pro ISO とライセンス）が未実施**で、[windows.md](windows.md) の手順で管理者が一度だけ作って `images.yaml` に登録する | [windows.md](windows.md) |
