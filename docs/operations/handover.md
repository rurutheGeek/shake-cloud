...
| — | net-01（i-88933be43f442c6f4） | 192.168.10.103 | Tailscale subnet router（N02。宅外から管理LANへの復旧経路） | 稼働。**Tailscale参加済み**（tailnet `100.91.7.69`。ルート承認・ACL・宅外検証が未了）。[net.md](net.md) |
...
| 🟨 | 外出先（Tailscale）から名前で使えるようにする | **2026-09-14: 復旧経路の subnet router `net-01`（`192.168.10.103`）を作成し、Tailscaleへ参加済み（`100.91.7.69`）**（N02・[net.md](net.md)）。ルート承認・ACL・宅外DNS検証が未了。過去の DNS SERVFAIL とサブネットルート未設定の解消は実測で確認する |
