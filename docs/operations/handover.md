...
| 🟨 | K8s | **スライス①〜④（ノード準備・kubeadm/Cilium・共通基盤・GitOps）を実機で確認（2026-09-12）。** cp-01 と worker-01 が **Ready**、Cilium 1.20.1 が kube-proxy を置換、PVC 永続化・MetalLB（`.240`）・cert-manager CA・**Flux 2.9.5 + SOPS の GitOps 同期**まで動作。作業ブランチは main へ取り込み済み（`8521834`）。次は **AWX**、CloudNativePG、Knative | [kubernetes.md](kubernetes.md) |
