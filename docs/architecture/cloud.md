...
| API（Phase 5） | 既存VMの引き取り | **実機確認済み（2026-09-11）**。`POST /v1/instances/adopt` でプール内の既存VMを登録。**game1（VMID 100）を `shunyazhiyuan97` として引き取り済み**。MAC・スペックの読み取り、重複 409、終了で残骸なしも確認。残りはポータルの仕上げとブートストラップ管理キーの無効化 |
