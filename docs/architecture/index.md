...
| 自作クラウドAPI・Terraform Provider・ポータル・CLI | API は Phase 4（ボリューム・セキュリティグループ）まで実装し、2026-09-11 に実機で確認。最小ポータルは稼働中（VM・ボリューム・SG 操作）。Terraform Provider・CLI・完成したセルフサービスポータルは未実装。手順は[クラウドAPIの構築](../operations/cloud.md) |
