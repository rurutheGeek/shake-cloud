...
**区分**: 運用改善・実機確認 ・ **状態**: 実測・軽量化・台帳反映済み。負荷組合せ試験（ゲーム・AI取り込み・AWX同時実行）は未実施
...
4. NetBoxのプロセス数、AWX並列数・履歴、メディア走査・変換、Ollama常駐、ログ保持、未使用検証環境を個別に改善する。重複する認証関連プロセスの停止は利用先の移行後だけ行う。
