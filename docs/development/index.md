...
更新日: 2026-09-12。状態: **計画書・開発用READMEを整備。並行作業のI01で実測・軽量化、I02でmedia-01作成、I05でサービスstateの資格情報境界、W01でHomarr新規スタック、H01でHome Assistant Containerを実装しservices-01へ配備してAuthentik SSO（hass-oidc-auth）まで設定（初回セットアップ・復元試験は未完）。H04でeufy-security-wsのstackを追加（Eufy資格情報待ち）。家電の実接続はH02のトークン取得待ち**。
...
- この資料整備の成果物は40件の計画書、開発ディレクトリのREADME、既存資料・図の整合です。並行する実装・実機作業の成果はI01・I02などの個別文書へ記録し、資料整備による実行と区別します。
...
詳細な状態・実装範囲・根拠・完了条件はリンク先が正本です。既存コードのある移行、新規開発、実機確認、資料修正、外部条件待ちを区別します。資料のみのDタスクも独立した成果物です。

| 計画 | 作業の性質 | 配備先・対象 |
| --- | --- | --- |
| [W01 Homarr](W01-homarr.md) | 移行 | services-01 |
| [W02 Vaultwarden](W02-vaultwarden.md) | 移行 | services-01 |
| [W03 Nextcloud・Calendar・Tasks](W03-nextcloud.md) | 移行 | media-01 |
| [W04 Kavita](W04-kavita.md) | 移行 | media-01 |
| [W05 Navidrome](W05-navidrome.md) | 移行 | media-01 |
| [W06 MeTube・音楽変換・Picard](W06-music-tools.md) | 移行・新規 | media-01 |
| [W07 RomM](W07-romm.md) | 新規 | game1 |
| [A01 ポケモンDB・WebUI・agent](A01-pokemon-ai.md) | 移行・参照元取得待ち | game1 |
| [A02 Ollama](A02-ollama.md) | 新規・実機検証 | game1 |
| [A03 汎用RAG](A03-rag.md) | 新規 | game1 |
| [A04 Discord Bot](A04-discord-bot.md) | 新規 | game1 |
| [A05 OpenHomeの製品・要件調査](A05-openhome.md) | 調査 | game1候補 |
| [G01 Wolf](G01-wolf.md) | 開発・実機検証 | game1 |
| [G02 Azahar・非公開ルーム](G02-azahar.md) | 開発・実機検証 | game1 |
| [G03 ゲームとAIの負荷調整](G03-game-ai-resources.md) | 実機検証・調整 | game1 |
| [H01 Home Assistant Container](H01-home-assistant.md) | 新規 | services-01 |
| [H02 SwitchBot](H02-switchbot.md) | 機器確認・新規 | services-01 |
| [H03 Echo](H03-echo.md) | 機器確認・新規 | services-01 |
| [H04 Eufy](H04-eufy.md) | 機器確認・調査 | services-01 |
| [N01 セルフホストVPN](N01-vpn.md) | 選定・新規 | services-01 |
| [N02 Tailscaleの復旧経路・DNS](N02-tailscale.md) | 既存経路の確認・改善 | 既存ラズパイ・端末 |
| [N03 VLAN切替](N03-vlan.md) | 宣言済み・実機切替待ち | 既存ネットワーク |
| [N04 公開Web入口](N04-public-edge.md) | 要件調査・新規 | public-edge候補 |
| [N05 既存サービスのHTTPS移行完了](N05-https.md) | 残作業 | services-01・接続元 |
| [I01 容量測定・軽量化](I01-resources.md) | 測定・改善 | 既存ホスト・VM |
| [I02 media-01のVM宣言](I02-media-vm.md) | 新規 | media-01 |
| [I03 クラウドVMのAnsible連携](I03-cloud-inventory.md) | 新規 | 配備用ツール |
| [I04 DNS登録](I04-cloud-dns.md) | 新規 | サービス宣言・DNS |
| [I05 サービス用state管理](I05-service-state.md) | 未決事項の具体化 | 既存外部state保存先 |
| [I06 AWXのジョブ整備](I06-awx.md) | 既存基盤への設定追加 | 既存Kubernetes |
| [O01 管理DBの外部バックアップ](O01-cloud-backup.md) | 既存ローカルバックアップの拡張 | cloud-01・既存外部保存先 |
| [O02 CNPGバックアップ](O02-cnpg-backup.md) | 新規 | 既存Kubernetes・Garage・外部保存先 |
| [O03 VM・アプリ状態の復元](O03-restore.md) | 既存手順の拡張・検証 | 対象VM・隔離復元先 |
| [D01 オンボーディング](D01-onboarding.md) | 資料更新 | docs |
| [D02 配置・配分表・構成図](D02-placement.md) | 資料更新 | docs |
| [D03 サービス配置とIaC所有境界](D03-service-boundaries.md) | 資料更新 | docs |
| [D04 実装状況の訂正](D04-status.md) | 資料更新 | docs |
| [D05 Picard](D05-picard.md) | 資料のみ | docs・media（音楽導線） |
| [D06 LocalSend](D06-localsend.md) | 資料のみ | docs |
| [D07 Tailcat](D07-tailcat.md) | 資料のみ | docs |
...
| D01–D07 | 実装・配備から独立して更新。未実施の確認を完了扱いにしない |

同じVMに配備すること自体は開発順序の依存ではありません。N05など共通基盤への依存は当該アプリに必要な入口・設定の準備を指し、他アプリを含む計画全体の完了を待つ条件ではありません。認証統合も各アプリで既存コードを再利用し、旧アカウント・subject・権限・クライアント接続を確認してから切り替えます。独立したアプリのDBを一括移行しません。
...
| 既存メディアCompose・同期スクリプト | W03–W06・W02が共有する。編集範囲を事前に分け、別ブランチまたはworktreeで作業して統合時に全関連サービスを検証 |
...
2026-09-12の検証: 厳格ビルド、40件のID・ナビゲーション対応、ローカルリンク、公開対象チェックが通過。構成図の文字幅・配置の修正は[D02](D02-placement.md)に残っており、この資料整備全体の完了条件は未達です。
