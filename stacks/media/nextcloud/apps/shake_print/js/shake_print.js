...
${message.stack}`;
  ...
  // src/print.js
  var PRINTABLE = ["pdf", "png", "jpg", "jpeg", "txt"];
  ...
  registerFileAction({
    id: "shake-print",
    displayName: () => translate("shake_print", "\u5370\u5237"),
    ...
    enabled: ({ nodes }) => nodes.length === 1 && PRINTABLE.includes((nodes[0].extension || "").toLowerCase().replace(/^\./, "")),
    exec: async ({ nodes }) => {
      const [node] = nodes;
      try {
        const { data } = await dist_default.post(generateUrl("/apps/shake_print/print"), {
          fileId: node.fileid
        });
        toast("success", translate("shake_print", "\u5370\u5237\u3057\u307E\u3057\u305F\uFF08{job}\uFF09", { job: data.job }));
        return true;
      } catch (error) {
        toast("error", error?.response?.data?.error || translate("shake_print", "\u5370\u5237\u306B\u5931\u6557\u3057\u307E\u3057\u305F"));
        return false;
      }
    },
    order: 25
