...
- **既存の確認:** アルバムを投入すると、保存先フォルダの既存MP3を数え、`保存済み N / 全 M 曲` と未取得数を確認画面に表示します（同じアルバムの再投入で、どこまで取得済みか分かる）。「続きからダウンロード」で再開します。
- **実行履歴は永続化**（`storage/khinsider/jobs.json`）。コンテナを再作成しても残り、中断したジョブは「再起動で中断」として失敗表示され「再試行」で再開できます。一覧は実行中がある間5秒ごとに自動更新します。
- **アルバム単位で最大3並列**（`KHINSIDER_WORKERS`、上限3）。同一アルバム内は直列で、同じアルバムを二重に走らせないようロックします。全ワーカー合計でリクエスト間隔（`KHINSIDER_REQUEST_INTERVAL`、既定0.4秒）を空けるので、並列でもCloudflareを踏みにくくしています。
- 実行にはコンテナから `downloads.khinsider.com`・配信ホスト・`musicbrainz.org`（任意）へ到達できる必要があります。

### Cloudflare（403 Forbidden）対策

KHInsiderはCloudflareのbot challenge配下にあり、**普通のHTTPクライアントだと `403 Forbidden`（`cf-mitigated: challenge`）** になります。UAをブラウザに変えても、TLS/HTTP2の指紋が違うとchallengeは解けません。

そこでコンテナ内の **curl_cffi**（MeTubeイメージがyt-dlp経由で同梱）を使い、ChromeのTLS/HTTP2指紋で接続します。1つのセッションを使い回してクリアランスCookieを曲ページ・配信URLへ引き継ぎ、`403`/`429`/`503` は待って再試行します（5→15→30→60秒）。**再試行のたびにセッションを作り直し、指紋（chrome/chrome131/firefox133）も切り替えます**（一度challengeされたセッションは待つだけでは回復しないことがあるため）。それでも通らないときは、ジョブ画面の **「再試行」** ボタンで同じアルバムをもう一度投入できます（保存済みの曲はスキップして続きから取得）。
...
日本語化せずに取得してしまった既存フォルダは、[retag-japanese.py](retag-japanese.py) で後から揃えられます（`ALBUMS` にフォルダ名とMusicBrainzの日本語盤IDを書いて、コンテナ内で `plan` → `apply`。旧名・旧タグのジャーナルつき）。
