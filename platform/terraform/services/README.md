...
現在のユニットは [media](media/README.md)（media-01。**apply済み・実機確認済み**。I02）と `monitor/`（monitor-01。監視スタック。M01）です。media-01 の Terraform 一式は `terraform validate` に加えて、実機へ apply して再 plan が No changes になることまで確認しています。監視スタックの仕様は [M01](../../../docs/development/M01-monitoring.md) を参照してください。
